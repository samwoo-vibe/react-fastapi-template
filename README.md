...
자동 배포되는 React 앱은 `frontend/nginx.conf`의 공통 정책을 그대로 사용합니다.
`index.html`과 SPA 경로는 항상 재검증하고, `/api/` 응답은 저장하지 않으며,
Vite가 파일명에 콘텐츠 해시를 붙인 `/assets/`만 1년 `immutable`로 캐시합니다.
따라서 새 버전 배포 뒤 일반 새로고침이나 즐겨찾기 진입에서도 최신 진입 문서를 받습니다.
파일명 해시가 없는 정적 파일에는 `immutable`을 적용하지 마세요.
